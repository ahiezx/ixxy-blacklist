...
if(isset($_POST["query"]))
{
 ...
 $sql2 = "SELECT name,account,platform FROM shops WHERE name LIKE '%".$search."%' AND special='0' OR account LIKE '%".$search."%' AND special='0' ORDER BY name LIMIT 3 ";
 $MySQL::exit();
}
...
if(mysqli_num_rows($result2) > 0)
{
 while($row = mysqli_fetch_array($result2))
 {
  ...
  $output .= "<li class='search_result'>".$platform."<a href='https://www.instagram.com/".$row["account"]."' target='_blank' class='text-white'>".$row["account"]."</a><p style='float:right;' class='text-success'>Trusted</p></li>";
 }
}
